...
parser.add_argument('--inter_ratio', default=0, type=float,
                    help='entrelacement ratio')

parser.add_argument('--SLCDI',  default=False, type=bool,
                    metavar='Label_smooth', help='smoothing labels')

parser.add_argument('--cutout',  default=False, type=bool,
                    metavar='cutout regularization', help='cutout regularization')

parser.add_argument('--fine_tune',  default=False, type=bool,
                    metavar='fine-tuning', help='Fine-tune from previous trained model')
...
def main():
    ...
    print("***********************************************************")
    print("train with Interlacing ratio =",args.inter_ratio)  
    print("***********************************************************")
    ...
    if args.dataset.startswith('cifar'):
        ...
        transform_list = [
            ...
            transforms.ToTensor()]
        if args.inter_ratio>0.0:
            transform_list.append( Interlacing(probability=.8,entre=args.inter_ratio))
            print("You are using CDI :) ")
        
        transform_list.append(normalize)
        if args.cutout:
            transform_list.append( Cutout(n_holes=1, length=8))
            print("***********************************************************")
            print("you are using CUTOUT")  
            print("***********************************************************")
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.beta > 0 and r < args.cutmix_prob:
            ...
            # compute output
            output = model(input)
            #loss = criterion(output, target_a)   Cutmiw without label Interpolation :)
